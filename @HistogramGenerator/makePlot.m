...
%       - Modified labeling: added Ppk, Mean, Sigma, FC; removed a few things
    ...
    if ~isempty(obj.Dist)
        ...
        xText = [xText {sprintf('Sample Size: %.0f   Failures: %.0f   Ppk: %.3f',obj.c.numDataPts,obj.c.numFail,obj.c.PpkNorm)}];
        % Add a line for the min/max/mean and std dev if normal distribution
        xText = [xText {sprintf('Min: %g   Max: %g   Mean: %g   Std: %g',obj.c.min,obj.c.max,obj.c.mean,obj.c.sigma)}];
        ...
        % Add a line for the min/max/mean and std dev if normal distribution
        xText = [xText {sprintf('Min: %g   Max: %g   Mean: %g   Std: %g',min(obj.Data),max(obj.Data),mean(obj.Data),std(obj.Data))}];
    end
