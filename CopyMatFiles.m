...
for i = 1: length(program)
    %     Destination = fullfile(DestRoot,DestFolder{i},'MatFiles');
    %     lastfilecopied = LastfileProcessed(Destination);
    %     datelastfile = str2double(lastfilecopied(end-9:end-4));
    %     startfolder = [datestr(datenum(num2str(datelastfilecopied),'yymmdd'),'yy-mm'), '_matfiles'];
    ...
    for j = 3:length(targetfolders)
        ...
        Destination = fullfile(DestRoot,DestFolder{i},truckname);
        lastfilecopied = Lastfileprocessed(Destination);
        if ~isempty(lastfilecopied)
            datelastfile = str2double(lastfilecopied(end-9:end-4));
            startfolder = [datestr(datenum(num2str(datelastfilecopied),'yymmdd'),'yy-mm'), '_matfiles'];
        else
            startfolder = [];
...
%         [sorted idx] = sort(char({tgtMatFolders(:).name}),'descend')
        for k = 3:length(tgtMatFolders)
