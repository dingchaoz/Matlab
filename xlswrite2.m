...
%   Modified to delete the three default sheets made for a new file and will run 
%   macro code to format the columns properly
%   
...
try
    ...
    function ExecuteWrite
            ...
            %%%%%%%%%%%%%%%%% Added
            % Delete the three default sheets if they exist
            try
                % Throws an error if the sheets do not exist.
                ExcelWorkbook.Worksheets.Item('Sheet1').Delete;
                ExcelWorkbook.Worksheets.Item('Sheet2').Delete;
                ExcelWorkbook.Worksheets.Item('Sheet3').Delete;
            catch ex
                % Do nothing, the sheets didn't exist so they don't need to be deleted
            end
            % Bold the header row
            ExcelWorkbook.ActiveSheet.Range('A1:K1').Font.Bold = true;
            % If there isn't already an auto-filter column
            if ~ExcelWorkbook.ActiveSheet.AutoFilterMode
                % Add a filter column to the header
                ExcelWorkbook.ActiveSheet.Range('A1').AutoFilter;
            end
            % Correct the style of the date field in the second column
            ExcelWorkbook.ActiveSheet.Columns.Item(2).NumberFormat = 'm/d/yyyy h:mm:ss AM/PM';
            % Auto-size the columns of the data
            for k = 1:11
                ExcelWorkbook.ActiveSheet.Columns.Item(k).EntireColumn.AutoFit;
            end
            % Default back to select cell A1
            ExcelWorkbook.ActiveSheet.Range('A1').Select;
            %%%%%%%%%%%%%%%%% End Added
            ExcelWorkbook.Save
    end
end
