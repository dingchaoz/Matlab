...
%   Revised - Dingchao Zhang - 2015/12/17
%       - Add lines to specify failure modes of minmax, event driven data
%       uploaded to tbltrucks
    ...
    %% Execute SQL query to fetch the last received file's datenum
    ...
    if (~exist('EventDriven_xSEID','var')|| ~exist('EventDriven_Data','var')) && (~exist('MinMax_PublicDataID','var')||~exist('MinMax_Data','var'))
        ...
        update(obj.conn, '[dbo].[tblTrucks]', {'EventData'}, {'No Evdd parameter'}, ...
        sprintf('WHERE [TruckID] = %0.f',truckID));
        ...
        % Update tblTrucks saying that there should have been MinMax data and wasn't
        update(obj.conn, '[dbo].[tblTrucks]', {'MinMaxData'}, {'No MinMax parameter'}, ...
        sprintf('WHERE [TruckID] = %0.f',truckID));
    ...
    elseif ~exist('MinMax_PublicDataID','var')
        ...
        obj.event.write(['No MinMax data in file ' fullFileName]);
        % Update tblTrucks saying that there should have been MinMax data and wasn't
        update(obj.conn, '[dbo].[tblTrucks]', {'MinMaxData'}, {'No MinMax_PublicDataID parameter'}, ...
        sprintf('WHERE [TruckID] = %0.f',truckID));
    ...
    elseif ~exist('MinMax_Data','var')
        ...
        obj.event.write(['No MinMax data in file ' fullFileName]);
        % Update tblTrucks saying that there should have been MinMax data and wasn't
        update(obj.conn, '[dbo].[tblTrucks]', {'MinMaxData'}, {'No MinMax_Data parameter'}, ...
        sprintf('WHERE [TruckID] = %0.f',truckID));
    ...
    if any(~strncmp('N',MinMax_PublicDataID,1))
        ...
        % Update MinMax to "No" when there is a new file in new day or the
        % last file of the day's minmax status is not "No".
        % This assures update MinMax to "No" only all the minmax files of
        % the day has no valid MinMax data
        if ~strcmp(TheLastFileMinMax,'Yes') || LastFileDatenum - TheLastFile > 1
            % Update tblTrucks saying that there should have been MinMax data and wasn't
            update(obj.conn, '[dbo].[tblTrucks]', {'MinMaxData'}, {'No'}, ...
            sprintf('WHERE [TruckID] = %0.f',truckID));
        end
