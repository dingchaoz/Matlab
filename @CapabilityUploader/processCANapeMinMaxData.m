function processCANapeMinMaxData(obj, abs_time, ECM_Run_Time, MMM_Update_Rate, Min_Max_PublicDataID, Min_Max_Data, cal, truckID)
...
%   
%   Original Version - Dingchao Zhang - ~Dec 10, 2014

    %% Initalize
    % Get the indicies of the valid MinMax data
    MinMax_PublicDataID = str2double(Min_Max_PublicDataID);
    MinMax_Data = str2double(Min_Max_Data);
    idxMinMaxOnly = find(~strncmp('',Min_Max_PublicDataID,1));
    ...
    while idx < numelMinMaxData
        ...
        setStartTime = abs_time;  % Starting datenum of this MinMax set
        ...
        % data (best way tosure, define a "MinMax set") (or have not passed the ending index)
...
%             end
            ...
            if ~isempty(MinMax_PublicDataID(globalIdx1))
                % If the current Public Data ID and next Public Data ID match
                if MinMax_PublicDataID(globalIdx)== MinMax_PublicDataID(globalIdx1)

                    % Get the public data id and decode both values
                        publicDataID = MinMax_PublicDataID(globalIdx);
                        ...
                        b = obj.decodeMinMax(publicDataID, MinMax_Data(globalIdx), cal);
                        ...
                        if b >= a || (isnan(a) && isnan(b))
                            ...
                            minMaxData(minMaxWriteIdx,:) = {abs_time, setECM_Run_Time, publicDataID, a, b, cal, truckID, [], 0, 0};
                            % Increment the write idx
                            minMaxWriteIdx = minMaxWriteIdx + 1;
                        else % b is the minimum and a is the maximum
                            % Create the entry in MinMax data here
                            minMaxData(minMaxWriteIdx,:) = {abs_time, setECM_Run_Time, publicDataID, b, a, cal, truckID, [], 0, 0};
                            % Increment the write idx
                            minMaxWriteIdx = minMaxWriteIdx + 1;
                    ...
                    publicDataID = hex2dec2(Min_Max_PublicDataID{globalIdx});
                    a = obj.decodeMinMax(publicDataID, MinMax_Data(globalIdx), cal);
...
%         end % while looking for MinMax data in this set
        ...
        if ~isempty(str2double(MMM_Update_Rate{globalIdx}))
            % Calculate the mean MMM_Update_Rate
%             meanMMM = mean(MMM_Update_Rate(~isnan(str2double(MMM_Update_Rate))));
            meanMMM = str2double(MMM_Update_Rate{globalIdx});
        else
