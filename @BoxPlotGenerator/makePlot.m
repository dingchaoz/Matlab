...
%       - Modified how the grouping and labels are calculated so as to have a common
%         function call for every plot type
%   Revised - Chris Remington - April 17, 2014
%       - Tried to add ability to do 2 grouping on the box plot, but could get it to work
%         correctly when specifying the labels differently from the group order input
...
%             obj.Labels2 = obj.Group2Order;
%         end
%         %numGroups2 = length(obj.Group2Order);
%     else
%         % Use the specified groups, calculate the number of groups present
%         %numGroups2 = length(obj.Group2Order);
%     end
    
    % Set the size of the outliers based on the group size
    if numGroups < 10
        outlierSize = 5;
    elseif numGroups < 20
        outlierSize = 4;
    ...
    % This code doesn't help any when making double groups
%     % Calculate labels for boxplot function
%     lables = cell(length(obj.Labels)*length(obj.Labels2),1);
%     for i = 1:length(obj.Labels)
%         for j = 1:length(obj.Labels2)
%             lables{length(obj.Labels2)*(i-1)+j} = [obj.Labels{i},',',obj.Labels2{j}];
%         end
%     end
%     groupOrder = cell(length(obj.GroupOrder)*length(obj.Group2Order),1);
%     for i = 1:length(obj.GroupOrder)
%         for j = 1:length(obj.Group2Order)
%             groupOrder{length(obj.Group2Order)*(i-1)+j} = [obj.GroupOrder{i},',',obj.Group2Order{j}];
%         end
%     end
    ...
    if visible
        ...
        figure('Name', ['Box Plot of ' obj.SystemErrorName],'defaulttextinterpreter','none');
    else
        % Otherwise make an invisible figure
        figure('Name', ['Box Plot of ' obj.SystemErrorName],'defaulttextinterpreter','none','visible','off');
    end
    
    % Make the shell of the box plot
    % If there is actually data present
    if ~isempty(obj.Data)
        % If there is no second grouping
        if 1%isempty(obj.Group2Data)
            % New common way to always plot the data if Labels and GroupOrder is always specified
            boxplot(obj.Data, obj.GroupData,'orientation','horizontal','labels',obj.Labels,'grouporder',obj.GroupOrder,'symbol','ko','outliersize',outlierSize,'factorseparator',1);
        else
            % Could never get two groups to work with box when specifiying the labels and
            % group order so that plots are sorted correctly
            boxplot(obj.Data, {obj.GroupData, obj.Group2Data},'orientation','horizontal','symbol','ko','outliersize',outlierSize,'factorseparator',1);
            % ,'labels',{obj.Labels,obj.Labels2},'grouporder',{obj.GroupOrder,obj.Group2Order}
        end
    else
        % Make a blank plot (IS THIS STILL NEEDED HERE? DATA SHOULD NEVER BE
        % EMPTY, OTHERWISE WHY SHOULD A PLOT BE MADE?)
        boxplot(obj.Data, obj.GroupData)
    end
