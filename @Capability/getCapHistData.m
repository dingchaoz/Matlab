function HistCapData = getCapHistData(obj, Name, varargin)
...
%   Revised - Chris Remington - April 9, 2014
%     - Modified date filtering so all conditions worked
...
%             select = 'SELECT [DataMin], [DataMax], [Engine]';
...
%             select = 'SELECT [datenum],[ECMRunTime],[DataMin],[DataMax],[TruckName],[Engine],[CalibrationVersion],[ConditionID]';
%     end
%     
    ...
    if ~isempty(p.Results.fields{1})
        ...
        if ~any(strcmp('All',p.Results.fields))
            ...
            % Select everything
            select = 'SELECT [CalibrationVersion],[TruckName],[Family],[FailureDataPoints],[StartDateStr],[EndDateStr],[Name],[Param],[LSLValue],[USLValue],[Mu],[Sigma],[PpK],[TimePeriod]';
        end
    else % if isnan(p.Results.grouping)
%         % Select all the fields (Default)
%         select = 'SELECT [datenum],[ECMRunTime],[DataMin],[DataMax],[TruckName],[Family],[Engine],[TruckType],[Rating],[CalibrationVersion],[ConditionID]';
    select = 'SELECT [CalibrationVersion],[TruckName],[Family],[DataPoints],[FailureDataPoints],[StartDateStr],[EndDateStr],[Name],[Param],[LSLValue],[USLValue],[Mu],[Sigma],[PpK],[TimePeriod]';
    end
    
    %% Generate the WHERE clause
    where = makeWhere(Name, p.Results);
    
    %% Fetch the data
    % Formulate the entire SQL query
    sql = [select ' FROM [dbo].[tblDataTruckTimeSw]  ' where ...
           ' ORDER BY [TruckName], [TimePeriod]'];
    
    % Move to the use of the common tryfetch to get the data
    HistCapData = obj.tryfetch(sql,100000);
    
end

function where = makeWhere(Name, args)
    ...
    where = sprintf('WHERE [Name] = %s', [char(39) Name char(39)] );
...
%     end
    ...
    switch args.truck
        ...
        otherwise
            % Throw an error as there was invalid input
            error('Capability:getCapHist:InvalidTruck','''truck'' input must be either ''all'', ''field'', or ''eng''');
    end
    ...
    if ~isempty(args.date)
        ...
        if length(args.date) == 2
            ...
            if isnan(args.date(1)) && ~isnan(args.date(2)) % Keep evenything up to the second date
                where = sprintf('%s And [EndDate] < ''%s''',where,datestr(args.date(2),29));
            ...
            elseif isnan(args.date(2)) && ~isnan(args.date(1)) % Keep everything after the fisrt date
                where = sprintf('%s And [StartDate] > ''%s''',where,datestr(args.date(1),29));
            elseif ~isnan(args.date(2)) && ~isnan(args.date(1)) % Nither was a NaN, use both value to filter between the range
                where = sprintf('%s And [StartDate] >= ''%s'' And [EndDate] < ''%s''',where,datestr(args.date(1),29),datestr(args.date(2),29));
            % else both were a NaN, don't do any filtering
            end
        else
            error('Capability:getCapHistData:InvalidInput', 'Invalid input for property ''date''')
        end
    end
