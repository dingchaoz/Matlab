...
%     - Changed the isLDD flag in dot2num but as stated above no action is needed here
    ...
    if ischar(calNumber)
        ...
        if isnan(calNumber)
            ...
            error('Capability:num2dot:InvalidInput','Input to calNumber must be a numeric value or single character array that could represent a number');
        end
    end
    
    if ~isnumeric(calNumber) || length(calNumber)~=1
        % Throw an error
        error('Capability:num2dot:InvalidInput','Input to calNumber must be a numeric value');
    end
    ...
    elseif calNumber >= 1000000 && calNumber <= 99999999
        ...
        % Throw an error
        error('Capability:num2dot:InvalidInput','The number input of %f couldn''t be interpreted into a software dot notation string as it must be between 6 and 8 digits long.',calNumber)
    end
