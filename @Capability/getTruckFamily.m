function truckFamily = getTruckFamily(obj,truckID)
%Returns family name given a truckID as input
%   
%   Usage: truckFamily = getTruckFamily(obj,truckID)
%   
%   Input -
%   truckID:     Numeric TruckID (same as stored in the database)
%   
%   Output -
%   truckFamily: String of the truck's name
%   
...
%       table
    ...
    if ~isempty(obj.tblTrucks)
        ...
        error('Capability:getTruckFamily:NoMatch', 'TruckID %.0f was not found in the database',truckID)
    end
    
    % If there was one and only one match
    if sum(idxTruck) == 1
        % Return that match
        truckFamily = obj.tblTrucks.Family{idxTruck};
        ...
        elseif sum(idxTruck) > 1
            ...
            error('Capability:getTruckFamily:TooManyMatches', 'More than one truck found with specified truckID: %.0f',truckID)
        else
