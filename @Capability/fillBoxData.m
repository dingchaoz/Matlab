function fillBoxData(obj, groupCode, group2Code)
...
%   set, and place it into the box object.
%   
%   Usage: fillBoxData(obj, groupCode, group2Code)
%   
%   Inputs - 
...
%   Revised - Chris Remington - April 16, 2014
%     - Added functionality to plot a second grouping if desired
    ...
    if isnan(obj.filt.ExtID)
        ...
        if isnan2(obj.box.LSL) && isnan2(obj.box.USL)
            % Plot both min and max data together
            obj.box.Data = [d.DataMin;d.DataMax];
            % Set the label for the data set that is being plotted
            obj.box.DataType = 'Min and Max Data';
        ...
        elseif isnan2(obj.box.USL)
            % Plot only the min data
            obj.box.Data = d.DataMin;
            % Set the label for the data set that is being plotted
            obj.box.DataType = 'Min Data';
            % Set the group data properly based on the groupCode
            assignGroupDataM(obj, d, groupCode, group2Code, false);
        % If this is an USL diagnostic
        elseif isnan2(obj.box.LSL)
            % Plot only the max data
            obj.box.Data = d.DataMax;
            % Set the label for the data set that is being plotted
            obj.box.DataType = 'Max Data';
            % Set the group data properly based on the groupCode
            assignGroupDataM(obj, d, groupCode, group2Code, false);
        ...
        obj.box.Data = d.DataValue;
        % Set the label for the data set that is being plotted
        obj.box.DataType = 'Event Driven Data';
        % Set the group data properly based on the groupCode
...
% Correctly assigns the group data based on the grouping code
    ...
    switch groupCode
        ...
        case 3 % Group data by month
            ...
            %obj.box.GroupData = cellstr([datestr(d.datenum, 'yy-mm_') datestr(d.datenum, 'mmmm yyyy')]);
            ...
            obj.box.GroupData = cellstr(num2str(vecs(:,1)+vecs(:,2)/20,'%.2f'));
            % Get the unique month and year combinations
            obj.box.GroupOrder = flipud(unique(obj.box.GroupData));
            ...
            obj.box.Labels = strcat(cellstr(datestr(labelDateVecs, 'mmmm')),cellstr(datestr(labelDateVecs, ' yyyy')));
            
            % As a side note, this fcn will give you unique year/month combinations
    ...
    switch group2Code
        ...
        case 0 % Group data by software version
            % Need to convert these to individual strings because software over 100000
            % causes %g to output group names in scientific notation confusing boxplot
            obj.box.Group2Data = cellstr(num2str(d.CalibrationVersion,'%.0f'));
            % Set the group data to be the calibration version
            %obj.box.GroupData = d.CalibrationVersion;
            % Set the group order and labels to their default
            obj.box.Labels2 = [];
        ...
        case 1 % Group data by truck name
            % Set the group data to be the truck name
            obj.box.Group2Data = d.TruckName;
            % Set the group order and labels to their default
            obj.box.Labels2 = [];
            obj.box.Group2Order = [];
        case 2 % Group data by family
            % Set the group data to be the family
            obj.box.Group2Data = d.Family;
            % Set the group order and labels to their default
            obj.box.Labels2 = [];
            obj.box.Group2Order = [];
        case 3 % Group data by month
            ...
            % Set the group data to be a string generated from the timestamp that
            % indicates the month that data was logged in
            %obj.box.Group2Data = cellstr([datestr(d.datenum, 'yy-mm_') datestr(d.datenum, 'mmmm yyyy')]);
            ...
            % Calculate a unique number calculation based on the month and year, this will
            % be the group value for each individual data point
            obj.box.Group2Data = cellstr(num2str(vecs(:,1)+vecs(:,2)/20,'%.2f'));
            % Get the unique month and year combinations
            obj.box.Group2Order = flipud(unique(obj.box.Group2Data));
            % Flush those out to fake date vectors
            uniqueYearMonthCalc = flipud(unique(vecs(:,1)+vecs(:,2)/20));
            labelDateVecs = [floor(uniqueYearMonthCalc) round(mod(uniqueYearMonthCalc,1)*20) ones(length(uniqueYearMonthCalc),4)];
            % Pull the result in to make the text label strings
            obj.box.Labels2 = strcat(cellstr(datestr(labelDateVecs, 'mmmm')),cellstr(datestr(labelDateVecs, ' yyyy')));
...
%             obj.box.GroupOrder = [];
    ...
    if double
        ...
        obj.box.GroupData = [obj.box.GroupData;obj.box.GroupData];
        obj.box.Group2Data = [obj.box.Group2Data;obj.box.Group2Data];
    end
end
